...
- For visual validation, display a static high-contrast screen with large OCR-friendly text before using a camera. The local acceptance target is the large `OK` text because it is more reliable with macOS Vision than dense pixel-font words or small digits. Avoid animated/random drawing for acceptance tests because it can look like display corruption. Use `make camera-aligner` to tune `CAMERA_CROP` and `OCR_ROTATE`, then pass those values to `make visual-smoke`. If the board is upside down in the camera, prefer `DISPLAY_ROTATION=2 make visual-smoke` so the display itself is upright.

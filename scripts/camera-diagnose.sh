...
if [[ "$CAMERA_DIAGNOSE_EXTRA_PROBES" == "1" ]]; then
  ...
  run_optional_section swift-capture-640x480 swift run --package-path "$ROOT_DIR" CameraSnapshot \
    --device "$CAMERA_DEVICE" \
    --output "$SWIFT_SMALL_IMAGE" \
    --timeout "$CAMERA_CAPTURE_TIMEOUT" \
    --size 640x480 \
    --format jpeg || SWIFT_SMALL_STATUS=$?

  if [[ "$CAMERA_DIAGNOSE_FFMPEG" == "1" ]]; then
    FFMPEG_SMALL_IMAGE="$DIAG_DIR/ffmpeg-capture-640x480-yuyv422.jpg"
    run_optional_section ffmpeg-capture-640x480-yuyv422 perl -e 'alarm shift; exec @ARGV' "$CAMERA_CAPTURE_TIMEOUT" ffmpeg \
      -hide_banner \
      -loglevel error \
      -f avfoundation \
      -framerate 30 \
      ...
      -i "$CAMERA_DEVICE:none" \
      -frames:v 1 \
      -y "$FFMPEG_SMALL_IMAGE" || FFMPEG_SMALL_STATUS=$?
  fi
...
if [[ "$SWIFT_STATUS" != "0" && "$FFMPEG_STATUS" != "0" && "$SWIFT_SMALL_STATUS" != "0" && "$FFMPEG_SMALL_STATUS" != "0" ]]; then

...
- Use `make camera-diagnose` when OCR capture fails. It records system camera inventory, Swift AVFoundation device status, related camera processes, avfoundation device listing, and video-only capture probes without touching audio input. If `capture_recommendation=no_video_frame_captured_close_camera_apps_or_reconnect_usb_camera`, do not keep rerunning board smokes; close camera-capable apps or reconnect the USB camera first.

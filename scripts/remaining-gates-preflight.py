...
GATES = [
    {
        ...
        "approval": "Allowed audio window; ES8311 output also needs supervised audible confirmation.",
        "evidence": "official_audio_physical_smoke_summary with failed=0 plus per-demo ES7210/ES8311 physical smoke lines.",
        "boundary": "Plan/preflight output is not physical audio evidence.",
    },
    {
        "id": "xiaozhi-ai",
        ...
        "destructive": "0",
        "audio": "0",
        "physical_followup": "XIAOZHI_READINESS_BACKUP=1 make xiaozhi-readiness && CONFIRM=--yes make xiaozhi-flash && make xiaozhi-runtime-visual-check",
        "approval": "Explicit user approval to replace the currently flashed Arduino sketch with XiaoZhi firmware.",
        "evidence": "fresh xiaozhi_backup_summary, xiaozhi_flash_summary, runtime onboarding/activation serial markers, and XiaoZhi AMOLED camera OCR.",
        "boundary": "Readiness, source build, and backup evidence are necessary rollback proof, not approval to flash.",
    },
    {
        "id": "audio-front-end",
        ...
        "approval": "Allowed audio window with permission to play host stimulus and use microphone capture.",
        "evidence": "hardware-smoke-suite audio-front-end pass with audio_summary RMS/peak serial evidence and log path.",
        "boundary": "audio-vad-preflight proves build/checker readiness only; it does not prove the microphone path.",
    },
    {
        "id": "web-ai-button",
        "reason": "external-required",
        ...
        "approval": "Human available to tap the AMOLED button during the smoke window.",
        "evidence": "WEB_AI_TOUCH_EVENT, WEB_AI_TRIGGER source=touch, WEB_AI_RESPONSE status=ok, and saved web-button evidence/logs.",
        "boundary": "Serial-triggered web-ai-button-smoke proves Wi-Fi/HTTP/display, but not the physical touch path.",
    },
]
...
def render_markdown(payload: dict[str, Any], summary_path: pathlib.Path) -> str:
    ...
    for item in results:
        gate = gate_by_id.get(str(item.get("id", "")), {})
        ...
        lines.append(
            "| {id} | {safe_scope} | {status} | `{log}` | {followup} |".format(
                ...
                status=status,
                log=log,
                followup=gate.get("physical_followup", item.get("physical_followup", "")),
            )
        )
    lines.extend(
        [
...
def latest_result_map(payload: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    if not payload:
        return {}
    results = payload.get("results", [])
...
def render_runbook(payload: dict[str, Any] | None, summary_path: pathlib.Path | None) -> str:
    result_by_id = latest_result_map(payload)
    latest_summary_text = f"`{rel(summary_path)}`" if summary_path else "`missing`"
    generated_at = payload.get("generated_at", "unknown") if payload else "unknown"
    lines = [
        ...
        "",
        f"Latest safe preflight summary: {latest_summary_text}",
        f"Latest safe preflight generated at: `{generated_at}`",
        "",
        "## Operator Rules",
        "",
    ...
    for gate in GATES:
        result = result_by_id.get(gate["id"], {})
        status = result.get("status", "missing")
        if result.get("skip_reason"):
            status = f"{status} ({result['skip_reason']})"
        log = rel(result.get("log", "")) if result.get("log") else "missing"
        lines.extend(
            [
                f"## {gate['id']}",
                "",
                f"- Blocking reason: `{gate['reason']}`",
                f"- Latest safe preflight: `{status}` via `{gate['safe_scope']}`",
                f"- Latest safe log: `{log}`",
                f"- Approval required: {gate['approval']}",
                f"- Supervised command: `{gate['physical_followup']}`",
                f"- Expected acceptance evidence: {gate['evidence']}",
                f"- Completion boundary: {gate['boundary']}",
                "",
            ]
        )
    lines.extend(
...
def main() -> int:
    ...
    parser.add_argument("--runbook", action="store_true", help="Render a supervised operator runbook from gate metadata and latest safe preflight status.")
    ...
    if args.runbook:
        ...
        if summary_path is not None:
            payload = json.loads(summary_path.read_text(encoding="utf-8"))
        print(render_runbook(payload, summary_path), end="")
        return 0
